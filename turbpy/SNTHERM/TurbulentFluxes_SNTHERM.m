function [qsen, qlat, Rb] = TurbulentFluxes_SNTHERM(Tair,Tsurface,RH,wsp,z0)
% Conversion of SNTHERM's (revision 4) qturb.f returns wind functions 
% qlat and qsen for the turbulent transfer of latent and sensible heat. All
% input values are assumed to be observed 2m above the snow's surface.
...
%
% SYNTAX:
%	[qsen, qlat, Rb] = TurbulentFluxes_SNTHERM(Tair,Tsurface,RH,wsp)
%
% INPUT:
%	Tair		= 1x1 scalar of air temperature [K]
%	Tsurface	= 1x1 scalar of snow surface temperature [K]
%	RH			= 1x1 scalar of RH [fractional]
...
%% Definitions (cause there's lots):
%
...
%% Checks
if numel(Tair) > 1 || numel(Tsurface) > 1 || numel(RH) > 1 || numel(wsp) > 1
	error('This function requires scalar arguments')
end
if max(RH) > 1
	error('Relative humidity should be a fraction')
end
...
rhoair = 100.*800/(287.*Tair);				% I'm assuming a high elevation site...
